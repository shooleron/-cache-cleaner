...
function KanbanCard({ task, isDragging }: { task: Task; isDragging?: boolean }) {
  ...
  const saveTitle = () => {
    ...
    if (title.trim() && title !== task.title)
      dispatch({ type: 'UPDATE_TASK', payload: { id: task.id, title: title.trim() } });
  };
  ...
  return (
    ...
    <div
      ...
      onClick={() => !editingTitle && dispatch({ type: 'OPEN_TASK_MODAL', payload: task.id })}
    >
      ...
      {editingTitle ? (
        ...
        <p className="kanban-card-title" onDoubleClick={e => { e.stopPropagation(); setTitle(task.title); setEditingTitle(true); }}>{task.title}</p>
      )}
      {task.dueDate && (
