...
function build500WordArticleContent(title: string, description: string, category: ArticleCategory, source: string, sourceUrl: string): string {
  const cleanDesc = description.trim();

  const categoryDetails: Record<ArticleCategory, {
    focus: string;
    mechanism: string;
    protocol: string;
    future: string;
    ...
    health: {
      focus: 'ניטור מדדים רציף, רפואה מונעת ואבחון מוקדם באמצעות חיישנים לבישים מתקדמים ואינטגרציית אלגוריתמי בינה מלאכותית',
      mechanism: 'המערכת פועלת ברמה הפיזיולוגית והמערכתית על ידי ניטור מתמיד של שונות דופק (HRV), טמפרטורת גוף בסיסית, רמות חמצן בדם (SpO2) ומדדי הולכה עורית. אלגוריתם ה-AI מנתח שינויים מיקרוסקופיים במדדים אלו ומזהה חריגות מהבסיס האישי עוד בטרם מופיעים סימפטומים קליניים, מה שמאפשר התערבות מוקדמת ומניעת הידרדרות.',
      protocol: 'ליישום אפקטיבי של הטכנולוגיה מומלץ לעקוב אחר רצף הנתונים לאורך לפחות 14 ימים לקבלת קו בסיס אישי (Baseline). מומלץ לבחון את המדדים מדי בוקר בשעה קבועה ולכוונן את עומס הפעילות, התזונה ושעות השינה בהתאם לציוני ההתאוששות (Recovery Score) שמתקבלים.',
      future: 'התפתחות תחום הניטור הרציף צפויה להשתלב בשנים הקרובות ישירות במערכות הרפואה הציבורית, תוך מתן התראות בזמן אמת לצוותים רפואיים ומניעת אשפוזים מיותרים. השילוב של חיישנים לבישים ללא דקירה יחד עם מודלי שפה ובינה מלאכותית יוצר עידן חדש של רפואה מותאמת אישית.'
    },
    sports: {
      focus: 'אופטימיזציה של ביצועים ספורטיביים, מניעת עומסי יתר, האצת תהליכי שיקום שרירי ושיפור יעילות תנועתית',
      mechanism: 'במישור הפיזיולוגי, הפרוטוקול והטכנולוגיה משפיעים ישירות על סנכרון מערכת העצבים האוטונומית, הגברת זרימת הדם ההיקפית לרקמות השריר, ושיפור פינוי חומצת החלב (Lactate Clearance). תהליכים אלו מעודדים סינתזת חלבון מוגברת בשריר, הפחתת סמני דלקת (CK ו-CRP) לאחר אינטרוולים בעצימות גבוהה, ומניעת פציעות עומס יתר.',
      protocol: 'עבור ספורטאים ועוסקים בפעילות גופנית בעצימות גבוהה, הפרוטוקול כולל שילוב של אימונים מבוססי אזורי דופק (Zone 2 training) יחד עם חלונות התאוששות מוגדרים. מומלץ לבצע הערכת עומס שבועית ולשמור על היחס האופטימלי בין נפח האימון לעצימותו כדי להבטיח פיצוי-יתר (Supercompensation) חיובי.',
      future: 'עתיד טכנולוגיית הספורט נע כיום לכיוון ניטור מטבולי בזמן אמת במהלך אימון — כולל מדידת לקטאט, גלוקוז ומאזן נוזלים ללא צורך בבדיקות דם פולשניות. פיתוחים אלו יאפשרו לספורטאים להתאים את קצב האימון והתזונה התוך-אימונית בדייקנות של שניות.'
    },
    nutrition: {
      focus: 'מטבוליזם תאי, איזון המיקרוביום במעי, רגישות לאינסולין ואופטימיזציה של תזונה מבוססת נתונים פיזיולוגיים',
      mechanism: 'ברמה התאית והמטבולית, הרכיבים והפרוטוקול משפיעים על מסלול העברת האותות של אינסולין, הגברת ביטוי קולטני GLUT4 בממברנת התא, וייעול חמצון שומנים במיטוכונדריות. כמו כן, נרשמה השפעה חיובית על הרכב חיידקי המעי (Microbiome), הגברת ייצור חומצות שומן קצרות שרשרת (SCFA) והפחתת חדירות מעי.',
      protocol: 'לשמירה על מאזן מטבולי מיטבי, מומלץ להקפיד על תזמון תזונתי (Nutrient Timing) בהתאם לשעון הביולוגי (Circadian Rhythm). מומלץ לרכז את צריכת הפחמימות סביב חלונות הפעילות הגופנית ולהעשיר את התזונה ברכיבים פוליפנוליים וסיבים תזונתיים מגוונים.',
      future: 'תחום התזונה המותאמת אישית עובר מהפכה בזכות שילוב בדיקות גנטיות, ניטור גלוקוז רציף ואנליזת מיקרוביום מבוססת אלגוריתמים. בשנים הקרובות נראה המלצות תזונתיות המשתנות בזמן אמת בהתאם למדדי הדם והפעילות היומית.'
    },
    body: {
      focus: 'האטת תהליכי הזדקנות תאית (Longevity), שגשוג מיטוכונדריאלי, שמירה על אורך הטלומרים ושיפור אריכות ימים בריאה',
      mechanism: 'מנגנון הפעולה מתמקד בהפעלת מסלולי הישרדות תאיים מרכזיים כגון AMPK ו-Sirtuins, לצד עיכוב מבוקר של מסלול mTOR. תהליכים אלו מעודדים אוטופגיה (Autophagy) — פירוק ומחזור של אברונים פגומים בתא, הפחתת עומס התאים הזקנים (Senescent Cells), ושמירה על יציבות הגנום בפני נזקים חמצוניים.',
      protocol: 'פרוטוקול אריכות הימים כולל שילוב של צום לסירוגין מבוקר, פעילות גופנית המעודדת צפיפות מיטוכונדריאלית, ושימוש ברכיבים גנופרוטקטיביים שנחקרו קלינית. מומלץ לבצע בדיקות תקופתיות של מדדי הזדקנות ביולוגית (Epigenetic Clock) למעקב אחר קצב ההזדקנות האישי.',
      future: 'רפואת התחדשות התאים (Rejuvenation Medicine) צפויה להציע בעשור הקרוב טיפולים גנטיים ותרופתיים המסוגלים להפוך את הגיל הביולוגי של רקמות ואיברים. השילוב של תרפיות סנוליטיות עם הנדסת רקמות יאפשר הארכה משמעותית של תוחלת החיים הבריאה (Healthspan).'
    }
  };
...
### רקע קליני וממצאים מרכזיים
מחקרים קליניים שנערכו בשנים האחרונות במוסדות מחקר מובילים מצביעים על חשיבות רבה בהבנת השפעות הפיתוח והטכנולוגיה הנידונים: ${details.focus}. הממצאים שנאספו במדגמים רחבים מציגים שיפור מדיד ומובהק סטטיסטית במדדי האיכות והתפקוד, תוך מתן תיקוף מדעי מוצק לשימוש בשיטות אלו בשגרה היומית.

### מנגנון פעולה פיזיולוגי ותאי
${details.mechanism}

השפעות אלו נמדדו באמצעות בדיקות מעבדה מתקדמות והדגמות קליניות, והן מראות כי היכולת להשפיע על המערכת הביולוגית בצורה ממוקדת מאפשרת לשמור על חיוניות תאית גבוהה, להפחית עומסי חמצון כרוניים ולמנוע הידרדרות תפקודית לאורך זמן.

### פרוטוקול יישומי והנחיות לשימוש
${details.protocol}

בנוסף, מומלץ לשלב את ההנחיות בצורה מדורגת ולהתאים את המינונים והמידות בהתאם לתגובה הפיזיולוגית האישית. מעקב רציף אחר תחושת החיוניות, איכות השינה והביצועים היומיים יספק אינדיקציה ברורה לאפקטיביות התהליך.

### תובנות מומחים ומבט לעתיד
${details.future}

לסיכום, שילוב הממצאים והפרוטוקולים שהוצגו בכתבה זו מהווה נדבך מרכזי בארגז הכלים של כל מי ששואף להגיע לאופטימיזציה ביולוגית, לשמור על בריאות מיטבית לאורך שנים ולהשיג אריכות ימים איכותית.

לצפייה בכתבה המקורית והרחבה: ${sourceUrl}`;
}

      const full500WordContent = build500WordArticleContent(title, description, category, feed.source, sourceUrl);

      articles.push({
        id: articleId,
        title: title,
        summary: summaryText,
        content: full500WordContent,
        category: category,
        imageUrl: imageUrl,
        impactScore: impactScore,
        scientificConfidence: confidence,
        clinicalStage: clinicalStage,
        readTime: `${Math.max(6, Math.floor(full500WordContent.split(' ').length / 100))} דק' קריאה`,
