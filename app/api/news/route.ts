...
function buildBounded100To500WordArticleContent(
  title: string,
  description: string,
  ...
  const categoryInserts: Record<ArticleCategory, string> = {
    health: 'ממצאי המחקר והטכנולוגיה הנדונה מדגישים את החשיבות הקריטית של ניטור מדדים רציף (שונות דופק HRV, טמפרטורת גוף בסיסית ואינטגרציית AI) לאבחון מוקדם ושיפור מנגנוני ההומיאוסטזיס בגוף.',
    sports: 'במישור הספורטיבי והפיזיולוגי, הפרוטוקול משפיע ישירות על סנכרון מערכת העצבים האוטונומית, מניעת עומסי יתר, הפחתת סמני דלקת בשריר והאצת תהליכי השיקום והפיצוי-היתר.',
    nutrition: 'ברמה המטבולית והתאית, ממצאים אלו מעידים על ייעול רגישות האינסולין, הגברת חמצון שומנים במיטוכונדריות, ושיפור מאזן חיידקי המעי (Microbiome) לשמירה על אנרגיה רציפה.',
    body: 'הפרוטוקול והממצאים מתמקדים באופטימיזציה של אריכות ימים בריאה (Longevity), הפעלת מסלולי הישרדות תאיים (AMPK ו-Sirtuins), עידוד אוטופגיה ושמירה על חיוניות תאית לאורך זמן.'
  };

  let content = cleanDesc;

  // Enforce Minimum 100 Words: Expand gracefully if too brief
  if (words.length < 100) {
    const extension = categoryInserts[category] || categoryInserts.health;
    content = `${cleanDesc}

### רקע ומנגנון פעולה פיזיולוגי
${extension} השילוב של נתונים מדויקים יחד עם מעקב קליני עקבי מאפשר להתאים פרוטוקולים אישיים ולמקסם את התגובה הביולוגית של הגוף.

### יישום מעשי והנחיות
לתוצאות מיטביות מומלץ לשלב את ההנחיות בצורה מדורגת, לעקוב אחר תגובת הגוף והמדדים האישיים לאורך זמן, ולכוונן את הפעילות והתזונה בהתאם.`;
  }

  // Enforce Maximum 500 Words: Cap neatly if too long
  const currentWords = content.split(/\s+/).filter(Boolean);
  if (currentWords.length > 450) {
    content = currentWords.slice(0, 420).join(' ') + '...';
  }

  return `${content}\n\nלצפייה בכתבה המקורית והרחבה: ${sourceUrl}`;
}

      const boundedContent = buildBounded100To500WordArticleContent(title, description, category, feed.source, sourceUrl);
      ...
      articles.push({
        ...
        readTime: `${Math.max(2, Math.floor(boundedContent.split(/\s+/).length / 100))} דק' קריאה`,
